...
pub fn api_logout(request: &HttpRequest) -> Result<HttpResponse, (HttpResponseCode, String)> {
    ...
    Ok(HttpResponse::new(
        ...
        HttpHeaders::builder()
            ...
            .set_cookie("session=; HttpOnly; Path=/; Max-Age=0".into())
            .build(),
        Some(r#"{"status": "success", "message": "Logged out successfully"}"#.as_bytes().to_vec().into_boxed_slice())
    ))
}